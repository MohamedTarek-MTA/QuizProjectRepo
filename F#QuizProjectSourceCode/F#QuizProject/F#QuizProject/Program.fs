...
let createQuizForm (userInfo: UserInfo) (questions: Map<int, Question>) =
    let form = new Form(Text = sprintf "Quiz - %s-%s DR.%s { 20 Marks }" userInfo.Name userInfo.ID userInfo.ProfName, Size = Size(800, 450), StartPosition = FormStartPosition.CenterScreen)
...
let createMainForm () =
    let form = new Form(Text = "F# Quiz", Size = Size(500, 450), StartPosition = FormStartPosition.CenterScreen)
